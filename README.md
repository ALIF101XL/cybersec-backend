...
CyberSec is a scalable collaborative search system being developed by [STIN](https://stin.ac.id/)

The backend is responsible for fetching search requests to the search provider and managing the application's data. 
It is built on NodeJS and exposes its endpoints through [express](https://expressjs.com/) (API) and [socket.io](https://socket.io/) (Websockets). Use it together with the [CyberSec Front End](https://github.com/ALIF101XL/cybersec-frontend) to get a web-based collaborative search interface.
...
- Set up the server
    ...
    git clone https://github.com/ALIF101XL/cybersec-backend.git
...
### Bing
CyberSec requires a [Bing API key](https://azure.microsoft.com/en-us/services/cognitive-services/bing-web-search-api/) to use the Bing web search provider.
...
# Modifications
CyberSec can be extended to define tasks, and to support new providers for search results.
...
The data structure of the `<result>` depends on the result type, which is defined by the component that will be used to display the result in the frontend. See the [cybersec-frontend documentation](https://github.com/felipemoraes/cybersec-frontend#search-providers) for an explanation of how to add custom result types.
...
# Citation
If you use CyberSec to produce results for your scientific publication, please refer to our [SIGIR 2018](http://fmoraes.nl/documents/moraes2018sigir.pdf) paper.

```
@inproceedings{putra2018searchx,
  title={CyberSec: Empowering Collaborative Search Research.},
